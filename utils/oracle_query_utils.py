...
def _generate_join_clause(joins):
    ...
    if len(joins) > 0:
        for join in joins:
            ...
            clause = f"JOIN {join_table}  {join_alias} ON {join_conditions}"
...
def _generate_where_clause(conditions):
    ...
    for cond_metadata in conditions:
        ...
        elif "booleanOperator" in cond_metadata:
            where_conditions.append(cond_metadata["booleanOperator"])
    where_condition = " ".join(where_conditions)
...
def generate_data_query(json_data):
    ...
    for entity in json_data:
        ...
        if not query:
            select_clause = ", ".join([f"{item['alias']}.{item['name']}" for item in entity["select"]])
            ...
            query = f"SELECT {select_clause} FROM {from_table} {table_alias} {join_clause} {where_clause} {order_by_clause}"
        queries.append({"tableName": table_Name, "query": str(query)})
    return queries
