...
from erp_connector.utils import global_context
...
def process_connector(db_connector, one_integration_client, erp_instance_id):
    ...
    try:
        ...
        output_folder_path = os.path.join(current_path, f'output_{command_id}')
        zip_folder_path = os.path.join(current_path, f'output_zip_{command_id}')
...
def verifying_config(config_path):
    ...
    with progress:
        ...
        global_context.one_integration_client = one_integration_client
        logger.info(f"Config loaded and client created")
        ...
        time.sleep(1)
        progress.update(task_load_config, advance=1)
        if custom_erp_connector_config.dbType!="netsuite":
