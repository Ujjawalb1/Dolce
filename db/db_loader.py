...
# from erp_connector.db.netsuite_connector import NetsuiteConnector
...
class ConnectorFactory:
    ...
    def create_connector(erp_connector_config,trace_id):
        ...
        # elif db_type == 'netsuite':
        #     logger.info("creating netsuite connector")
        #     publish_event(get_request_data(trace_id, "", "", ""), logs)
        #     return NetsuiteConnector(connection_details)
        else:
            raise ValueError("Invalid db connector config: dbName must be 'mssql' or 'postgresql'")
